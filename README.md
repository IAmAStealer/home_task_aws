...
- **Terraform modules**: the root `main.tf` instantiates two modules, kept separate because they answer different questions:
  - `modules/health-api/` — *what* gets shipped: API Gateway, Lambda, DynamoDB, KMS, CloudWatch, and the Lambda's own IAM role.
  - `modules/github-ci/` — *how* it gets deployed: the GitHub OIDC provider and the CI/CD deploy role (`oidc.tf`, `iam.tf` inside that module). It takes the ARNs it needs to scope its IAM policy to as inputs (`module.health_api.*` outputs, wired in the root module) rather than reaching into `health_api`'s resources directly — the two modules don't know about each other.
  A single `modules/everything/` module, or per-resource modules (one for DynamoDB, one for KMS, etc.), were both considered and rejected: the former doesn't separate "app" from "deploy plumbing" at all, and the latter splits resources that have no independent reuse case (the Lambda role, the table, the key are only ever used together), adding variable/output plumbing for no benefit.
...
- **`kms:ListAliases` on `Resource: "*"`** (`modules/github-ci/iam.tf`, deploy role) — this action has no resource-level scoping in AWS's IAM model at all (there is no resource type to restrict it to), so `"*"` is the only valid value. Suppressed with `#tfsec:ignore:aws-iam-no-policy-wildcards`.
